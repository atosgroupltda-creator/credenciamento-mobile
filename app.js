(() => {
  ...
  function saveLeadName(name){
    ...
    sessionStorage.setItem("lead_full_name", clean);
    sessionStorage.setItem("lead_first_name", firstName(clean));
  }
  ...
  function runProcessing(){
    ...
    const timer = setInterval(() => {
      ...
      if(index < stages.length){
        ...
        setTimeout(() => {
          $("#leadName").textContent = sessionStorage.getItem("lead_first_name") || "Visitante";
          showScreen("aprovado");
        },650);
      }
    },1100);
  ...
  const saved = sessionStorage.getItem("lead_full_name");
